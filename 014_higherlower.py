...
def format_data(candidate):
    """Takes candidate data and returns printable format"""
    name = candidate["name"]
    description = candidate["description"]
    country = candidate["country"]
    return f"{name}, a {description} from {country}"

def check(choice, followers_a, followers_b):
    """Takes the user guess and follower counts """
...
while game_continues:
    ...
    print(f"\nCompare A: {format_data(candidate_a)}")
    print(vs)
    ...
    is_correct = check(choice, followers_a, followers_b)
    if is_correct:
        score += 1
